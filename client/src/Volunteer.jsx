...
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
  {volunteers.map((volunteer) => (
    <div 
      ...
      className="cursor-pointer group transition-all duration-300 hover:scale-105 w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-full mx-auto"
    >
      <div
        className="relative overflow-hidden rounded-xl shadow-md h-[95%] lg:h-full"
        style={{ backgroundColor: '#FFFEF7' }}
      >
        {/* Responsive image height */}
        <div className="relative pt-[80%] sm:pt-[85%] lg:pt-[110%] overflow-hidden">
          <img
            src={volunteer.image}
            alt={volunteer.name}
        ...
        {/* Text Info */}
        <div className="p-3 sm:p-4 lg:p-5 min-h-[96px] lg:min-h-[110px] flex flex-col justify-start">
          <h3 className="font-bold text-base sm:text-lg lg:text-xl" style={{ color: '#2C2C2C' }}>
            {volunteer.name}
          </h3>
          <p className="text-sm sm:text-base mb-1" style={{ color: '#C41E3A' }}>
            {volunteer.title}
          </p>
          <p className="text-xs sm:text-sm lg:text-base" style={{ color: '#6B4B4B' }}>
            {volunteer.role}
          </p>
        </div>

        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3 lg:p-5">
          <span className="text-white text-sm lg:text-base font-medium">View Profile →</span>
        </div>
      </div>
    </div>
  ))}
